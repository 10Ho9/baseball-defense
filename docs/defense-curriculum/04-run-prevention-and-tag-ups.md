...
- 핵심 움직임:
  ...
  - `P`는 홈 뒤 백업을 돈다.
- 백업/정렬:
  - `LF`는 3루선 뒤를 먼저 잡아 left-side 누수와 다음 3루 진루를 정리한다.
  - `RF`는 1루선 뒤를 준비해 fallback 1루 악송구와 우측 누수를 막는다.
  - `CF`는 2루 뒤로 짧게 들어와 가운데 큰 리바운드와 다음 베이스 전환을 정리한다.
- 흔한 실수:
...
- 백업/정렬:
  ...
  - `LF`는 3루선 뒤를 먼저 잡아 Home 플레이 뒤 좌측 누수와 추가 진루를 막는다.
  - `RF`는 1루선 뒤를 준비해 후속 1루 전환 악송구와 우측 누수를 정리한다.
  - `CF`는 2루 뒤로 짧게 들어와 가운데 큰 리바운드와 다음 주자 진루를 동시에 관리한다.
- 흔한 실수:
...
- 핵심 움직임:
  - `LF`는 캐치 우선 후 공을 몸 안쪽으로 통과시키며 Home 방향으로 송구 발을 만든다.
  - `C`는 태그 라인을 준비한다.
  - `P`는 Home 뒤 백업을 돈다.
